/* Node unit test for the dashboardV2 strategy mappers (adapt.js) — 单一模型 v2:
...
// Mirror of poly_fight/follow_strategy.py validate_follow_strategy (v2 单一模型).
function backendErrors(s) {
  ...
  if (!fp(sz.per_signal_percent)) e.push("sizing.per_signal_percent");
  if (!fp(sz.per_match_percent)) e.push("sizing.per_match_percent");
  else if (+sz.per_match_percent + 1e-9 < +sz.per_signal_percent) e.push("sizing.per_match_percent");
...
const k0 = { usableMode: "cap", usableCap: "2000", minSignalOn: true, minSignal: "10", maxEntryOn: true, maxEntry: "0.68", perSignalPct: "1", perMatchPct: "2", minStake: "1", realtimeRefresh: false };
const s0 = A.strategyFromKit(k0, 2000);
eq("s0.schema", s0.schema_version, 2);
...
// ---- strategyExample (示例推演:flat 单笔 = 余额×per_signal%,夹到 [min, 每场预算]) ----
const base = { usableMode: "all", usableCap: "", minSignalOn: false, minSignal: "100", maxEntryOn: true, maxEntry: "0.68", perSignalPct: "1", perMatchPct: "2", minStake: "1" };
const ex = (over, sample, wallet) => A.strategyExample(Object.assign({}, base, over), sample, wallet);
eq("ex.flat", ex({ perSignalPct: "1" }, "1200", 5000).amount, 50);          // 1% of 5000
eq("ex.flat_ignores_sample", ex({ perSignalPct: "1" }, "99999", 5000).amount, 50);  // 与目标下单额无关
eq("ex.budget_cap", ex({ perSignalPct: "5", perMatchPct: "2" }, "1200", 5000).amount, 100); // 5%=250 → 预算 2%=100
eq("ex.min_floor", ex({ perSignalPct: "0.001", minStake: "5" }, "1200", 5000).amount, 5);
eq("ex.ignored_below_threshold", ex({ minSignalOn: true, minSignal: "100" }, "50", 5000), { ignored: true });
eq("ex.threshold_disabled", ex({ minSignalOn: false }, "50", 5000).amount, 50);

// ---- strategyIssues (必填校验 / 启动门槛) ----
const valid = { usableMode: "cap", usableCap: "5000", minSignalOn: true, minSignal: "10", maxEntryOn: true, maxEntry: "0.68", perSignalPct: "1", perMatchPct: "2", minStake: "1" };
const iss = (over, wallet) => A.strategyIssues(Object.assign({}, valid, over), wallet === undefined ? 8000 : wallet);
eq("iss.valid_empty", iss({}), []);
eq("iss.missing_perSignal", iss({ perSignalPct: "" }), ["单笔基数%"]);
eq("iss.missing_perMatch", iss({ perMatchPct: "" }), ["单场预算%"]);
eq("iss.budget_lt_signal", iss({ perSignalPct: "5", perMatchPct: "2" }), ["单场预算不能小于单笔基数"]);
...
console.log("ALL PASS (strategy mappers v2 + example + issues + library)");
process.exit(0);
